  ...
  return (
    <section className="relative flex min-h-[92vh] items-center overflow-hidden brand-gradient-radial pt-20 text-white">
      <Image
        ...
        alt="Crowds exploring the Automation Expo trade floor"
        fill
        priority
        sizes="100vw"
      ...
      <div className="relative mx-auto grid w-full max-w-7xl grid-cols-1 gap-10 px-4 py-16 sm:px-6 lg:grid-cols-12 lg:px-8">
        <div className="lg:col-span-8">
          <span className="inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-gold ring-1 ring-white/20 backdrop-blur">
            <span className="size-2 rounded-full bg-cta animate-pulse" /> {EVENT.tagline}
          </span>

          <h1 className="mt-5 font-heading text-4xl font-bold leading-[1.05] text-balance sm:text-6xl lg:text-7xl">
            Automation <span className="text-gradient-gold">Expo 2026</span>
          </h1>
          <p className="mt-4 max-w-xl text-lg text-white/80 text-pretty sm:text-xl">
            India&apos;s premier B2B trade show for robotics, machine vision and smart manufacturing. Three days. Five zones. Endless opportunity.
          </p>

          <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm font-medium text-white/90">
            <span className="inline-flex items-center gap-2"><CalendarDays className="size-5 text-gold" /> {EVENT.dateLabel}</span>
            <span className="inline-flex items-center gap-2"><MapPin className="size-5 text-gold" /> {EVENT.venue}</span>
          </div>

          <div className="mt-8">
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.16em] text-white/60">Doors open in</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <CtaButton variant="glass" size="lg" onClick={() => open('exhibitor')}>
        ...
        <div className="hidden lg:col-span-4 lg:block">
          <div className="grid grid-cols-2 gap-3">
            {[
              { k: '40K+', v: 'Trade visitors' },
              { k: '500+', v: 'Exhibitors' },
              { k: '5', v: 'Tech zones' },
              { k: '80+', v: 'Speakers' },
            ].map((s) => (
              <div key={s.v} className="glass rounded-2xl p-5">
                <p className="font-heading text-3xl font-bold text-gold">{s.k}</p>
